...
export default function TableOrderScreen() {
  ...
  const [orderKind, setOrderKind] = useState<"table" | "p2p">("table");
  ...
  const reload = useReload(async () => {
    ...
    setOrder(ord);
    if (table) setTableName(table.name);
    if (table) setOrderKind(table.type === "p2p" ? "p2p" : "table");
    if (user) setShiftId((await getOpenShift(user.id))?.id ?? null);
  });
  ...
  useEffect(() => {
    navigation.setOptions({
      title: orderKind === "p2p" ? `Customer: ${tableName}` : tableName,
    });
  }, [navigation, tableName, orderKind]);

  // If it's a P2P order and the user leaves without adding anything, discard
  // the empty virtual order instead of leaving it behind.
  useEffect(() => {
    const unsub = navigation.addListener("beforeRemove", () => {
      if (orderKind === "p2p" && order.length === 0) {
        deleteEmptyP2POrder(tableId).catch(console.error);
      }
    });
    return unsub;
  }, [navigation, orderKind, order.length, tableId]);
  ...
  const subtotal = useMemo(
    () => order.reduce((s, o) => s + o.price * o.quantity, 0),
    [order],
  );
              ...
              {bill.serviceCharge > 0 && (
                <View style={styles.sumRow}>
                  <Text style={{ color: colors.textMuted }}>
                    Service charge ({bill.serviceRate}%)
                  </Text>
                  <Text style={{ color: colors.text }}>
